...
interface ListingFormProps {
    ...
    assetImages: string[];
    onCancel: () => void;
    onSave: (draft: ListingDraft, intent: SaveIntent) => Promise<void>;
}
...
function normalizeDateInputValue(value: string | undefined) {
    if (!value) {
        return "";
    }
...
export function ListingForm({ mode, initialListing, assetImages, onCancel, onSave }: ListingFormProps) {
    ...
    const handleSave = async (intent: SaveIntent) => {
        ...
        try {
            await onSave(normalizeDraft(formState, intent), intent);
        } catch (error) {
            if (error instanceof ApiError) {
                setSubmitError(error.message);
            } else {
                setSubmitError("Unable to save listing right now.");
            }
        } finally {
    ...
    return (
        <div className="space-y-5">
            <section className={sectionClassName}>
                <div className="mb-5">
                    <h2 className="font-display text-[1.55rem] leading-tight text-slate-950">Basics</h2>
                </div>
            ...
            <section className={sectionClassName}>
                <div className="mb-5">
                    <h2 className="font-display text-[1.55rem] leading-tight text-slate-950">Photos</h2>
                </div>
            ...
            <section className={sectionClassName}>
                <div className="mb-5">
                    <h2 className="font-display text-[1.55rem] leading-tight text-slate-950">Specs</h2>
                </div>
                    ...
                    {specFields.map((field) => (
                        <label key={field.key} className="grid gap-2">
                            ...
                            {field.inputType === "date" ? (
                                <input
                                    type="date"
                                    value={normalizeDateInputValue(formState.specs[field.key])}
            ...
            <section className={sectionClassName}>
                <div className="mb-5">
                    <h2 className="font-display text-[1.55rem] leading-tight text-slate-950">Inquiry</h2>
                </div>
                ...
                <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
                    ...
                    <div className="rounded-[1.2rem] border border-stone-200 bg-stone-50/85 p-4">
                        <span className="text-[13px] font-medium text-slate-700">WhatsApp preview</span>
            ...
            <section className={sectionClassName}>
                <div className="mb-5">
                    <h2 className="font-display text-[1.55rem] leading-tight text-slate-950">Display</h2>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <ToggleField
                        ...
                        description="Show this listing in the public inventory."
                        onChange={(value) => setFormState((current) => ({ ...current, showOnWebsite: value }))}
                    />
                    <ToggleField
                        label="Mark as featured"
                        value={formState.featured}
                        description="Keep this listing near the top."
                ...
                <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-end">
                        <div className="flex flex-col gap-2.5 sm:flex-row">
                            ...
                            >
                                {activeIntent === "publish" ? "Saving..." : mode === "create" ? "Create listing" : "Save changes"}
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                {activeIntent === "draft" ? "Saving..." : "Save draft"}
                            </button>
                            <button
                                type="button"
                                onClick={onCancel}
